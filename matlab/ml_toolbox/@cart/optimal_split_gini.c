		   ...
		   int *no_optimal)
{
	...
	right_sum += right_total[i];
    }

    best_Q = 1.0 - best_Q / (right_sum * right_sum);
	...
	for (j=0; j<(data_length-1); j++) {
	    ...
	    for (k=0; k<cat; k++) {
		left_dens[i] = left_total[i] / left_sum;
		right_dens[i] = right_total[i] / right_sum;
	    }
	    ...
	    right_Q = 1.0;
	    for (k=0; k<cat; k++) {
		left_Q -= left_dens[i] * left_dens[i];
		right_Q -= right_dens[i] * right_dens[i];
	    }

	    /* Calculate our new impurity */
	    this_Q = (left_Q * left_sum + right_Q * right_sum) / w_sum;
		   ...
		   int *no_optimal)
{
	...
	for (j=0; j<(data_length-1); j++) {
	    ...
	    /*	    mexPrintf("i = %d, j = %d, this_y = %d, this_w = %g\n",
		    i, j, this_y, this_w); */
	    ...
	    /* Convert our totals into densities (very expensive...) */
	    for (k=0; k<cat; k++) {
		left_dens[k] = left_total[k] / left_sum;
		right_dens[k] = right_total[k] / right_sum;
	    }
