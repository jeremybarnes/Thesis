...
less clear: the low-noise datasets \ds{ring0} and \ds{sonar} show a
straight curve with no obvious minima (the slope indicating that such
a minima may exist for $p > 2$); whereas the curves for \ds{wpbc} and
\ds{acacia} both appear to involve a random process.
...
\ds{wpbc} dataset was difficult also, with only about one half of the
